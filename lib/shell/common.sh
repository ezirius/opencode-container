...
__env_OPENCODE_LABEL_NAMESPACE="${OPENCODE_LABEL_NAMESPACE-}"
__env_OPENCODE_LABEL_WORKSPACE="${OPENCODE_LABEL_WORKSPACE-}"
__env_OPENCODE_LABEL_LANE="${OPENCODE_LABEL_LANE-}"
__env_OPENCODE_LABEL_UPSTREAM="${OPENCODE_LABEL_UPSTREAM-}"
__env_OPENCODE_LABEL_UPSTREAM_REF="${OPENCODE_LABEL_UPSTREAM_REF-}"
__env_OPENCODE_LABEL_WRAPPER="${OPENCODE_LABEL_WRAPPER-}"
__env_OPENCODE_LABEL_COMMITSTAMP="${OPENCODE_LABEL_COMMITSTAMP-}"
...
__env_OPENCODE_LANE_PRODUCTION="${OPENCODE_LANE_PRODUCTION-}"
__env_OPENCODE_LANE_TEST="${OPENCODE_LANE_TEST-}"
__env_OPENCODE_DEFAULT_UPSTREAM_SELECTOR="${OPENCODE_DEFAULT_UPSTREAM_SELECTOR-}"
__env_OPENCODE_UPSTREAM_MAIN_SELECTOR="${OPENCODE_UPSTREAM_MAIN_SELECTOR-}"
__env_OPENCODE_UPSTREAM_DEV_SELECTOR="${OPENCODE_UPSTREAM_DEV_SELECTOR-}"
__env_OPENCODE_RELEASE_TAG_PREFIX="${OPENCODE_RELEASE_TAG_PREFIX-}"
__env_OPENCODE_BASE_ROOT="${OPENCODE_BASE_ROOT-}"
__env_OPENCODE_DEVELOPMENT_ROOT="${OPENCODE_DEVELOPMENT_ROOT-}"
__env_OPENCODE_WORKSPACE_HOME_DIRNAME="${OPENCODE_WORKSPACE_HOME_DIRNAME-}"
__env_OPENCODE_WORKSPACE_DIRNAME="${OPENCODE_WORKSPACE_DIRNAME-}"
__env_OPENCODE_WORKSPACE_CONFIG_SUBDIR="${OPENCODE_WORKSPACE_CONFIG_SUBDIR-}"
...
__env_OPENCODE_MANAGED_SERVER_HOSTNAME="${OPENCODE_MANAGED_SERVER_HOSTNAME-}"
__env_OPENCODE_MANAGED_SERVER_CONTAINER_PORT="${OPENCODE_MANAGED_SERVER_CONTAINER_PORT-}"
__env_OPENCODE_SERVER_HEALTHCHECK_PATH="${OPENCODE_SERVER_HEALTHCHECK_PATH-}"
__env_OPENCODE_RUNTIME_SERVER_PID_FILENAME="${OPENCODE_RUNTIME_SERVER_PID_FILENAME-}"
__env_OPENCODE_RUNTIME_SERVER_LOG_FILENAME="${OPENCODE_RUNTIME_SERVER_LOG_FILENAME-}"
...
[[ -z "$__env_OPENCODE_LABEL_NAMESPACE" ]] || OPENCODE_LABEL_NAMESPACE="$__env_OPENCODE_LABEL_NAMESPACE"
[[ -z "$__env_OPENCODE_LABEL_WORKSPACE" ]] || OPENCODE_LABEL_WORKSPACE="$__env_OPENCODE_LABEL_WORKSPACE"
[[ -z "$__env_OPENCODE_LABEL_LANE" ]] || OPENCODE_LABEL_LANE="$__env_OPENCODE_LABEL_LANE"
[[ -z "$__env_OPENCODE_LABEL_UPSTREAM" ]] || OPENCODE_LABEL_UPSTREAM="$__env_OPENCODE_LABEL_UPSTREAM"
[[ -z "$__env_OPENCODE_LABEL_UPSTREAM_REF" ]] || OPENCODE_LABEL_UPSTREAM_REF="$__env_OPENCODE_LABEL_UPSTREAM_REF"
[[ -z "$__env_OPENCODE_LABEL_WRAPPER" ]] || OPENCODE_LABEL_WRAPPER="$__env_OPENCODE_LABEL_WRAPPER"
[[ -z "$__env_OPENCODE_LABEL_COMMITSTAMP" ]] || OPENCODE_LABEL_COMMITSTAMP="$__env_OPENCODE_LABEL_COMMITSTAMP"
...
[[ -z "$__env_OPENCODE_LANE_PRODUCTION" ]] || OPENCODE_LANE_PRODUCTION="$__env_OPENCODE_LANE_PRODUCTION"
[[ -z "$__env_OPENCODE_LANE_TEST" ]] || OPENCODE_LANE_TEST="$__env_OPENCODE_LANE_TEST"
[[ -z "$__env_OPENCODE_DEFAULT_UPSTREAM_SELECTOR" ]] || OPENCODE_DEFAULT_UPSTREAM_SELECTOR="$__env_OPENCODE_DEFAULT_UPSTREAM_SELECTOR"
[[ -z "$__env_OPENCODE_UPSTREAM_MAIN_SELECTOR" ]] || OPENCODE_UPSTREAM_MAIN_SELECTOR="$__env_OPENCODE_UPSTREAM_MAIN_SELECTOR"
[[ -z "$__env_OPENCODE_UPSTREAM_DEV_SELECTOR" ]] || OPENCODE_UPSTREAM_DEV_SELECTOR="$__env_OPENCODE_UPSTREAM_DEV_SELECTOR"
[[ -z "$__env_OPENCODE_RELEASE_TAG_PREFIX" ]] || OPENCODE_RELEASE_TAG_PREFIX="$__env_OPENCODE_RELEASE_TAG_PREFIX"
...
[[ -z "$__env_OPENCODE_MANAGED_SERVER_HOSTNAME" ]] || OPENCODE_MANAGED_SERVER_HOSTNAME="$__env_OPENCODE_MANAGED_SERVER_HOSTNAME"
[[ -z "$__env_OPENCODE_MANAGED_SERVER_CONTAINER_PORT" ]] || OPENCODE_MANAGED_SERVER_CONTAINER_PORT="$__env_OPENCODE_MANAGED_SERVER_CONTAINER_PORT"
[[ -z "$__env_OPENCODE_SERVER_HEALTHCHECK_PATH" ]] || OPENCODE_SERVER_HEALTHCHECK_PATH="$__env_OPENCODE_SERVER_HEALTHCHECK_PATH"
[[ -z "$__env_OPENCODE_RUNTIME_SERVER_PID_FILENAME" ]] || OPENCODE_RUNTIME_SERVER_PID_FILENAME="$__env_OPENCODE_RUNTIME_SERVER_PID_FILENAME"
[[ -z "$__env_OPENCODE_RUNTIME_SERVER_LOG_FILENAME" ]] || OPENCODE_RUNTIME_SERVER_LOG_FILENAME="$__env_OPENCODE_RUNTIME_SERVER_LOG_FILENAME"
...
[[ -n "${OPENCODE_LABEL_NAMESPACE:-}" ]] || fail "missing OPENCODE_LABEL_NAMESPACE in config/shared/opencode.conf"
[[ -n "${OPENCODE_LABEL_WORKSPACE:-}" ]] || fail "missing OPENCODE_LABEL_WORKSPACE in config/shared/opencode.conf"
[[ -n "${OPENCODE_LABEL_LANE:-}" ]] || fail "missing OPENCODE_LABEL_LANE in config/shared/opencode.conf"
[[ -n "${OPENCODE_LABEL_UPSTREAM:-}" ]] || fail "missing OPENCODE_LABEL_UPSTREAM in config/shared/opencode.conf"
[[ -n "${OPENCODE_LABEL_UPSTREAM_REF:-}" ]] || fail "missing OPENCODE_LABEL_UPSTREAM_REF in config/shared/opencode.conf"
[[ -n "${OPENCODE_LABEL_WRAPPER:-}" ]] || fail "missing OPENCODE_LABEL_WRAPPER in config/shared/opencode.conf"
[[ -n "${OPENCODE_LABEL_COMMITSTAMP:-}" ]] || fail "missing OPENCODE_LABEL_COMMITSTAMP in config/shared/opencode.conf"
...
[[ -n "${OPENCODE_LANE_PRODUCTION:-}" ]] || fail "missing OPENCODE_LANE_PRODUCTION in config/shared/opencode.conf"
[[ -n "${OPENCODE_LANE_TEST:-}" ]] || fail "missing OPENCODE_LANE_TEST in config/shared/opencode.conf"
[[ -n "${OPENCODE_DEFAULT_UPSTREAM_SELECTOR:-}" ]] || fail "missing OPENCODE_DEFAULT_UPSTREAM_SELECTOR in config/shared/opencode.conf"
[[ -n "${OPENCODE_UPSTREAM_MAIN_SELECTOR:-}" ]] || fail "missing OPENCODE_UPSTREAM_MAIN_SELECTOR in config/shared/opencode.conf"
[[ -n "${OPENCODE_UPSTREAM_DEV_SELECTOR:-}" ]] || fail "missing OPENCODE_UPSTREAM_DEV_SELECTOR in config/shared/opencode.conf"
[[ -n "${OPENCODE_RELEASE_TAG_PREFIX:-}" ]] || fail "missing OPENCODE_RELEASE_TAG_PREFIX in config/shared/opencode.conf"
...
[[ -n "${OPENCODE_MANAGED_SERVER_HOSTNAME:-}" ]] || fail "missing OPENCODE_MANAGED_SERVER_HOSTNAME in config/shared/opencode.conf"
[[ -n "${OPENCODE_MANAGED_SERVER_CONTAINER_PORT:-}" ]] || fail "missing OPENCODE_MANAGED_SERVER_CONTAINER_PORT in config/shared/opencode.conf"
...
lane_usage_text() {
	printf '<%s|%s>' "$OPENCODE_LANE_PRODUCTION" "$OPENCODE_LANE_TEST"
}

is_lane() {
	local lane="$1"
	[[ "$lane" == "$OPENCODE_LANE_PRODUCTION" || "$lane" == "$OPENCODE_LANE_TEST" ]]
}

upstream_selector_usage_text() {
	printf "'%s', '%s', or an exact release tag" "$OPENCODE_UPSTREAM_MAIN_SELECTOR" "$OPENCODE_DEFAULT_UPSTREAM_SELECTOR"
}
...
workspace_home_dir() {
	local workspace="$1"
	printf '%s/%s' "$(workspace_root_dir "$workspace")" "$OPENCODE_WORKSPACE_HOME_DIRNAME"
}

workspace_dir() {
	local workspace="$1"
	printf '%s/%s' "$(workspace_root_dir "$workspace")" "$OPENCODE_WORKSPACE_DIRNAME"
}

workspace_config_dir() {
	local workspace="$1"
	printf '%s/%s' "$(workspace_dir "$workspace")" "$OPENCODE_WORKSPACE_CONFIG_SUBDIR"
}
...
opencode_server_port_publish_spec() {
	...
	container_port_publish_spec "$configured_port" "$OPENCODE_MANAGED_SERVER_CONTAINER_PORT" 'OPENCODE_HOST_SERVER_PORT'
}
...
container_config_dir() {
	printf '%s/%s' "$(container_workspace_dir)" "$OPENCODE_WORKSPACE_CONFIG_SUBDIR"
}
...
validate_upstream_selector() {
	...
	[[ "$selector" == "$OPENCODE_UPSTREAM_MAIN_SELECTOR" || "$selector" == "$OPENCODE_DEFAULT_UPSTREAM_SELECTOR" || "$selector" =~ ^${OPENCODE_RELEASE_TAG_PREFIX}?[0-9]+\.[0-9]+\.[0-9]+$ ]] || fail "upstream selector must be $(upstream_selector_usage_text)"
}
...
release_tag_display() {
	local tag="$1"
	printf '%s' "${tag#${OPENCODE_RELEASE_TAG_PREFIX}}"
}

list_release_tags() {
	local raw_tag display_tag seen_tags=""
	while IFS= read -r raw_tag; do
		[[ -n "$raw_tag" ]] || continue
		display_tag="$(release_tag_display "$raw_tag")"
		contains_line "$seen_tags" "$display_tag" && continue
		if [[ -n "$seen_tags" ]]; then
			seen_tags+=$'\n'
		fi
		seen_tags+="$display_tag"
		printf '%s\n' "$display_tag"
	done < <(api_get "$OPENCODE_GITHUB_API_BASE/repos/anomalyco/opencode/releases" | python3 -c 'import json, sys
...
latest_release_tag() {
	local raw_tag
	raw_tag="$(api_get "$OPENCODE_GITHUB_API_BASE/repos/anomalyco/opencode/releases/latest" | python3 -c 'import json, sys
data = json.load(sys.stdin)
tag = (data.get("tag_name") or "").strip()
if not tag:
    raise SystemExit("failed to resolve latest OpenCode release")
print(tag)')"
	release_tag_display "$raw_tag"
}
...
resolve_upstream_selector() {
	...
	case "$selector" in
	"$OPENCODE_UPSTREAM_MAIN_SELECTOR") printf '%s' "$OPENCODE_UPSTREAM_MAIN_SELECTOR" ;;
	"$OPENCODE_DEFAULT_UPSTREAM_SELECTOR") latest_release_tag ;;
	${OPENCODE_RELEASE_TAG_PREFIX}*) printf '%s' "${selector#${OPENCODE_RELEASE_TAG_PREFIX}}" ;;
	*) printf '%s' "$selector" ;;
	esac
}
...
upstream_ref_for_selector() {
	...
	if [[ "$resolved" == "$OPENCODE_UPSTREAM_MAIN_SELECTOR" ]]; then
		printf '%s' "$OPENCODE_UPSTREAM_MAIN_SELECTOR"
	else
		printf '%s%s' "$OPENCODE_RELEASE_TAG_PREFIX" "$resolved"
	fi
}

select_upstream_selector() {
	local choices=("$OPENCODE_UPSTREAM_MAIN_SELECTOR")
	local tag
...
select_lane() {
	local choices=("$OPENCODE_LANE_PRODUCTION" "$OPENCODE_LANE_TEST")
	local selection
	selection="$(select_menu_option "Select a build lane" "${choices[@]}")"
	printf '%s' "${choices[$((selection - 1))]}"
}
...
current_wrapper_context() {
	...
	if git_is_primary_worktree "$workdir"; then
		...
		if [[ -z "$current_branch" || "$current_branch" == "$OPENCODE_UPSTREAM_MAIN_SELECTOR" ]]; then
			printf '%s' "$OPENCODE_UPSTREAM_MAIN_SELECTOR"
		else
...
validate_build_context() {
	...
	if [[ "$lane" == "$OPENCODE_LANE_PRODUCTION" ]]; then
		git_is_primary_worktree "$workdir" || fail "$OPENCODE_LANE_PRODUCTION builds must run from the canonical $OPENCODE_UPSTREAM_MAIN_SELECTOR checkout"
		[[ "$(git_current_branch "$workdir")" == "$OPENCODE_UPSTREAM_MAIN_SELECTOR" ]] || fail "$OPENCODE_LANE_PRODUCTION builds must run from branch '$OPENCODE_UPSTREAM_MAIN_SELECTOR'"
		git_has_tracking_branch "$workdir" || fail "$OPENCODE_LANE_PRODUCTION builds require a tracking branch"
		git_is_in_sync_with_upstream "$workdir" || fail "$OPENCODE_LANE_PRODUCTION builds require the canonical $OPENCODE_UPSTREAM_MAIN_SELECTOR checkout to be in sync with its upstream"
	fi
}

validate_generic_build_context() {
	local workdir="${1:-${ROOT:-$(pwd)}}"
	[[ "${OPENCODE_SKIP_BUILD_CONTEXT_CHECK:-}" == "1" ]] && return 0
	require_clean_worktree "$workdir"
}
...
container_server_port_matches_workspace_config() {
	...
	actual_port="$(host_port_for_container_port "$container_name" "$OPENCODE_MANAGED_SERVER_CONTAINER_PORT" 2>/dev/null || true)"
...
server_url_for_container() {
	...
	port="$(host_port_for_container_port "$container_name" "$OPENCODE_MANAGED_SERVER_CONTAINER_PORT" 2>/dev/null || true)"
	[[ -n "$port" ]] || return 1
	printf 'http://127.0.0.1:%s' "$port"
}

server_healthcheck_path() {
	printf '%s' "$OPENCODE_SERVER_HEALTHCHECK_PATH"
}
...
clone_upstream_source() {
	...
	require_git
	if [[ "$ref" == "$OPENCODE_UPSTREAM_DEV_SELECTOR" || "$ref" == "$OPENCODE_UPSTREAM_MAIN_SELECTOR" ]]; then
		git clone --depth 1 --branch "$OPENCODE_UPSTREAM_MAIN_SELECTOR" "$OPENCODE_REPO_URL" "$destination" >/dev/null 2>&1 || fail "failed to clone upstream $OPENCODE_UPSTREAM_MAIN_SELECTOR"
		return 0
	fi
...
build_source_image() {
	...
	cp "$ROOT/config/containers/Containerfile.source-base.template" "$temp_dir/config/containers/Containerfile.source-base.template"
	...
	cp "$temp_dir/config/containers/Containerfile.source-base.template" "$source_dockerfile"
