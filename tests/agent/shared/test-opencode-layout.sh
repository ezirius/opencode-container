...
grep -q 'When passed `--publish`, `opencode-run` prints the published server URL and host port after creating or starting the published container\.' "$ROOT/docs/usage/shared/usage.md"
grep -q 'When passed `--publish`, `opencode-run` also opens the published server URL in the default browser on macOS and Linux after creating or starting the published container\.' "$ROOT/docs/usage/shared/usage.md"
