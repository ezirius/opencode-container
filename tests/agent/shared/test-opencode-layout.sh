...
grep -q 'By default, `opencode-run` publishes host port `4096 + workspace offset`' "$ROOT/docs/usage/shared/usage.md"
grep -q 'serve --hostname 0.0.0.0 --port 4096' "$ROOT/docs/usage/shared/usage.md"
grep -q 'By default, `opencode-run` also opens the published server URL in the default browser on macOS and Linux\.' "$ROOT/docs/usage/shared/usage.md"
